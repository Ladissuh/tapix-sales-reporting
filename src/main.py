...
def fetch_and_persist(week_label, week_monday):
    ...
    print(f"Staženo {len(deals)} dealů.")
    open_deals  = [d for d in deals if (d.get("properties") or {}).get("dealstage") not in closed_won_ids | closed_lost_ids]
    closed_deals= [d for d in deals if (d.get("properties") or {}).get("dealstage") in  closed_won_ids | closed_lost_ids]
    deal_ids = [d.get("id") for d in closed_deals]
    cid_by_deal = hs.get_deal_company_ids(token, deal_ids)
    cnames      = hs.get_company_names(token, list(cid_by_deal.values()))
    snap_rows = []
    for d in open_deals:
        ...
        snap_rows.append({"week_label": week_label, "week_monday": week_monday.isoformat(),
    ...
    for d in closed_deals:
        p = d.get("properties") or {}; sid = p.get("dealstage"); oid = p.get("hubspot_owner_id")
        try: amt = float(p.get("amount") or 0)
        except ValueError: amt = 0.0
        ...
        ledger_rows.append({"deal_id": d.get("id"), "deal_name": p.get("dealname",""),
            "company_name": cnames.get(cid,""), "owner_name": owners_map.get(str(oid),"Unassigned"),
            "closedate": cd_raw, "amount": amt,
            "outcome": "won" if sid in closed_won_ids else "lost", "week_label": close_wl})
...
def build_report(week_monday):
    ...
    snap_dates = pd.to_datetime(snap["closedate"], errors="coerce").dt.date if not snap.empty else None
    snap_eoy = snap[snap_dates < eoy] if not snap.empty else snap
    ...
    owners = display_order; z = [0]*len(week_labels)
    wb = rb.new_workbook()
    ...
    for owner in owners:
        ...
        rows = metrics.build_owner_report_rows(
            wf.get(owner,{s:z for s in stage_order}), we.get(owner,{s:z for s in stage_order}),
    ...
    agg_rows = metrics.build_owner_report_rows(agg_wf, agg_we, agg_won, agg_lost, agg_wc, agg_lc,
                                               stage_order, total_goal or None)
    lb = sorted(lb_totals.items(), key=lambda x: -x[1])
    rb.build_aggregation_sheet(wb, agg_rows, stage_order, week_labels_display, week_labels_display, lb)
